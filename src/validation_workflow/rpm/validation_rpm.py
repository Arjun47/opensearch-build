...
class ValidateRpm(Validation, DownloadUtils):
    ...
    def validation(self) -> bool:
        test_result, counter = ApiTestCases().test_apis(self.args.version, self.args.projects, self.check_for_security_plugin(os.path.join("usr", "share", "opensearch"), "rpm") if not self.args.force_https_check else True)
        if (test_result):
