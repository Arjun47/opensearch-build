...
class ValidateYum(Validation, DownloadUtils):
    ...
    def installation(self) -> bool:
        try:
            execute('sudo rpm --import https://artifacts.opensearch.org/publickeys/opensearch.pgp',
                    str(self.tmp_dir.path), True, False)
            for project in self.args.projects:
                ...
                execute(
                    f"sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD={get_password(str(self.args.version))} yum install '{project}-{self.args.version}' -y",
                    ".")
    ...
    def validation(self) -> bool:
        test_result, counter = ApiTestCases().test_apis(self.args.version, self.args.projects,
                                                        self.check_for_security_plugin(
                                                            os.path.join(os.sep, "usr", "share", "opensearch"),
                                                            "yum") if not self.args.force_https_check else True)  # noqa: E501
        if (test_result):
